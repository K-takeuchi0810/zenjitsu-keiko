...
class SchedulerBatchTests(unittest.TestCase):
    def test_scheduled_task_uses_no_pause(self):
        ...
        self.assertNotIn("(Get-Date).AddDays(1).ToString('yyyyMMdd')", batch)
        self.assertIn("next_available_race_date", batch)
        self.assertIn("No future race program found in DB", batch)
        self.assertIn("collect_trends.py --date %RESULT_DATE% --no-publish", batch)
        self.assertIn("Skipping GitHub Pages publish and phone notification", batch)
        self.assertIn("Result date rule: today", batch)
        self.assertIn("Next recommendation date rule: first DB race date after", batch)
        ...
        self.assertIn("Data mining fetch skipped because no next recommendation date was found.", batch)
